...
\begin{tasks}
  ...
  \PrintSolutionsTF{ \fbox{\parbox{\linewidth}{
    ...
    \textbf{SPNE$_1$}:
    \begin{itemize}
      \item Student chooses $honest$
      \item Faculty $expels$s
      \item Board $expel$s more than 50\% of the cases refered to them
    \end{itemize}
    If the faculty chooses to $refer$, 
    there is still a possibility the threat of expulsion
    will incentivize the student to stay $honest$.
    Suppose the $q<1/2$ so the faculty always $refer$s:
    \begin{align*}
      EU_s(honest) & > EU_s(cheat) \\
      \Rightarrow 2 & > 0q + 3(1-q) \\
      2 & > 3 - 3q \\
      q & > 1/3 \\
    \end{align*}
    \textbf{SPNE$_2$}:
    \begin{itemize}
      \item Student chooses $honest$
      \item Faculty $refer$s
      \item Board $expel$s $\frac{1}{3} < q < \frac{1}{2}$ percent of the cases.
    \end{itemize}
  }}}{\vspace{4cm}}
  \task (\points{4}) 
...
\end{center}
  \begin{tasks}
    ...
    \PrintSolutionsTF{ \fbox{\parbox{\linewidth}{
      ...
      \begin{align*}
        EU_{\text{Manager}}(\texttt{Executive}|\text{College}) & 
        = 10\left(\frac{1\times 1/2}{1/2 + 1/2(r)}\right) 
        + 0 \left(\frac{r\times 1/2}{1/2 + 1/2(r)}\right) 
        = 10 \frac{1}{1+r} \\
        EU_{\text{Manager}}(\texttt{Janitor}|\text{College}) & 
        = 5\left(\frac{1\times 1/2}{1/2 + 1/2(r)}\right) 
        + 10 \left(\frac{r\times 1/2}{1/2 + 1/2(r)}\right) 
        = 5 \frac{1}{1+r} + 10 \frac{r}{1+r} \\
      \end{align*}
      So a Manager will be willing to mix their hiring 
      between executives and janitors when:
      $$10 = 5 + 10r \Rightarrow r = 1/2$$
      ...
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          = $\frac{(1)(1/2)}{(1)(1/2)+(1/2)(1/2)} = \frac{1}{3/2} = 2/3$
        \end{itemize}
    ...
    Use Bayes rule to compare the ex-ante probability $p=1/2$ of a Type A
    ...
    \PrintSolutionsTF{ \fbox{\parbox{\linewidth}{
      ...
      The signal of college gives the Manager a better idea of the Employee's true type 
      than their naive estimate of $p=1/2$.
      $$

\begin{tikzpicture}[scale=1.5,font=\footnotesize, edge from parent/.style={draw, very thick}]
    ...
    \node(0)[hollow node,label=above:{$N_1$}]{}
        child{node(1)[solid node,label=above left:{$N_2$}]{}
            child{node({T1})[label=below:{\color{crimson} $T_1$}]{} edge from parent node[left]{$i$}}
            child{node(T2)[label=below:{\color{crimson} $T_2$}]{} edge from parent node[right]{$j$}}
            edge from parent node[left,xshift=-5]{$A$}
        }
        child{node(2)[solid node,label=above right:{$N_3$}]{}
            child{node(T3)[label=below:{\color{crimson} $T_3$}]{} edge from parent node[left]{$k$}}
            child{node(T4)[label=below:{\color{crimson} $T_4$}]{} edge from parent node[right]{$l$}}
    ...
    \draw[<-, color=crimson](T1)--+(-.5,-.1)node[left, color=crimson]{Terminal Nodes};
    \draw[<-, color=crimson](T2)--+(-.5,-.1);
    \draw[<-, color=crimson](T3)--+(-.5,-.1);
    \draw[<-, color=crimson](T4)--+(-.5,-.1);

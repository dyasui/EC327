\begin{frame}{Imperfect Info: Dealing with Risk}
  We've already seen how to deal with strategic uncertainty in \textit{mixed strategies}
  \begin{itemize}
    \item \alert{Expected Utility} tells us how people value stochastic outcomes
    \item $EU(L) = \sum_{o\in L} u(o) \times p(o)$
    \item how much utility you earn from each potential outcome
    \item multiplied by how likely each outcome is
  \end{itemize}
\end{frame}
...
\begin{frame}{Playing with Chance}
  Suppose that Nature plays probabilistically with 50:50 odds:
  \begin{center}
...
\begin{frame}{Certainty Equivalent Calculation}
  What is Zoidberg's certainty equivalent of Let it Ride?
  \begin{itemize}
    \item $EU_Z(L) = 0.5\sqrt{16} + 0.5\sqrt{0} = 2$
    \item What payment of $x$ would make Zoidberg indifferent?
    \item $u_Z(x) = EU_Z(L)$
  \end{itemize}
\end{frame}